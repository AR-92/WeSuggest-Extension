var palette = [
    ...
    ["#515151", "#FFFFFF", "#00B4FF", "#EEEEEE", "#00B4FF"],
    ...
    ["#E65540", "#F8ECC2", "#65A8A6", "#79896D", "#79896D"],
    ...
    ['#AF1D49', '#FC437B', '#FCF6AF', '#21A3C9', '#2690AF'],
    ['#ECF4F8', '#20C2E7', '#0578C6', '#EB8079', '#FBC782'],
    ['#C0E0EA', '#888EEF', '#675DDD', '#A044E3', '#DB5AE6'],
    ['#C9FBFF', '#17BEBB', '#EF2D1F', '#F2E2BA', '#EFBC9B'],
    ['#03738C', '#F2F2F2', '#F2DC99', '#F29F05', '#F20707'],
    ['#BDBDBD', '#890D0E', '#D91C0F', '#FEFEFE', '#484848'],
    ['#6BB8F5', '#84C4F5', '#9BCCF2', '#B5D9F5', '#CEE4F5'],
    ['#F0D736', '#F1A71D', '#F2612F', '#45887B', '#F4F2EC'],
    ['#965F06', '#FFEC9D', '#007341', '#7D0019', '#000075'],
    ['#FFD700', '#B30F84', '#FF00B6', '#14CACC', '#09B1B3'],
    ['#CEF19E', '#A7DDA7', '#78BE80', '#39A774', '#0B5F4C'],
    ['#007C00', '#003C00', '#00BC00', '#00C900', '#00A200']
];
...
function getRandomColor(svatch) {
    return svatch[Math.floor(Math.random() * svatch.length - 1) + 1]
}

function getRandomColorFun(svatch) {
    return svatch[Math.floor(Math.random() * svatch.length - 1) + 1]
}

function getSinColor(svatch) {
    var seconds = new Date().getTime() / 0.1;
    var i = Math.floor(Math.sin(seconds) * (svatch.length - 1));
    return svatch[Math.abs(i)];
}
function getSingleColor(svatch){
    var seconds = new Date().getTime() / 10;
    var i = Math.floor(Math.sin(seconds) * (svatch.length - 1));
    return svatch[Math.abs(i)];  
}
function getSingleColor1(svatch){
    var seconds = new Date().getTime() / 0.00001;
    var i = Math.floor(Math.sin(seconds) * (svatch.length - 1));
    return svatch[Math.abs(i)];  
}
function getRandomHex() {
    var randomColorHex = Math.floor(Math.random() * 16707200).toString(16);
    return "#" + randomColorHex;
}

function getrandomPallet() {
    return [getRandomHex(), getRandomHex(), getRandomHex(), getRandomHex(), getRandomHex()]
}
var gg = 0;
var g = getRandomColor(palette);
var t = getRandomColorFun(colorFun);
var y = 0.05;

var az = parseFloat((Math.random() * (0.006 - 0.001) + 0.001).toFixed(3))
var aw = parseInt((Math.random() * (40 - 200) + 200))
var aq = parseInt((Math.random() * (1 - 255) + 255))
var as = parseInt((Math.random() * (1 - 3) + 3))
    // var as=parseInt((Math.random() * (1 - 5) + 5))
var ad = parseFloat((Math.random() * (0.1 - 1.00) + 1.00).toFixed(2))
var randomColorHex = Math.floor(Math.random() * 16777215).toString(16);
...
var sxxx=storeSettings.getStore("settings");
if(sxxx.background.show) {
function setup() {
// if(sxxx.general.dark_mode){
//     backcolor=0;
// }else{
//     backcolor=255;
// }
        ...
        // strokeWeight(ad);
        stroke('#000000')
        let tx = floor(width / 2);
        let ty = floor(height / 2);
        pos = createVector(tx, ty);
    }
  
    var maxnoise = parseFloat((Math.random() * (0.0 - 0.70) + 0.70).toFixed(2));
    
    function cusShape(posx, posy, s1) {
        // let s1=20
        s1=Math.abs(s1)
        // console.log("cus ",s1)
        beginShape();
        for (var j1 = 0; j1 <= 360; j1 += 4) {
            let xoff1 = map(cos(j1 + posx), -1, 1, 0, maxnoise);
        let yoff1 = map(sin(j1 + posy), -1, 1, 0, maxnoise);
        //   let r1 = s1;
        let r1 = map(noise(xoff1, yoff1), 0, 1, 1, s1);
        // let r = map(noise(xoff, yoff), 0, 1, rad-i, (rad * 2)-i);
        let a1 = r1 * cos(j1);
        let b1 = r1 * sin(j1);
        vertex(a1 + posx, b1 + posy);
    }
    endShape(CLOSE);
};
...
function raper(value) {
    if(pos.x <=0 ) {pos.x =0}
    if(pos.y <=0 ) {pos.y =0}
    if(pos.y >=height ) {pos.y =height}
    if(pos.x >=width ) {pos.x =width}
    // if (pos.x > width ) { pos.x = value*2; }
    // if (pos.y > height ) { pos.y = value*2; }
...
function draw() {
    ...
    // var ax = parseFloat((Math.random() * (0.01 - 0.100) + 0.100).toFixed(3))
    // var ay = parseFloat((Math.random() * (0.01 - 0.050) + 0.050).toFixed(3))
    // var as = parseFloat((Math.random() * (0.1 - 1.00) + 1.00).toFixed(2))
    
    for (var i = 0; i < 10; i++) {
        // for (var i = 0; i < aw; i++) {
            var step = createVector(map(noise(xoff), 0, 1, -width, width), map(noise(yoff), 0, 1, -height, height))
            step.mult(0.0005);
            // step.mult(az);
            pos.add(step);
            // pos.add(xoff/100);
            var c = color(t(g));
            // var c = color(t(getrandomPallet()));
            // var c = color(getRandomHex());
            stroke(c)
            
            var r = floor(map(noise(xoff), 1, 0, -400, 400));
            raper(r);
            // var c = color(getRandomColor(g));
            fill(backcolor);
            // fill(c);
            // circle(pos.x,pos.y,r/as);
            // cusShape(pos.x, pos.y, 100)
            cusShape(pos.x, pos.y, r)
            xoff = xoff+0.00151;
            // xoff = xoff + ax;
            gg++;
        }
